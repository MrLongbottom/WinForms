...
namespace WindowsFormsSolution.Models
{
    class TestCases
    {
		...
		public TestCases(Database database)
        {
            ...
            //add test Project
            database.addProject(new Project(database.users.First<User>(), "Hankats svimmingpool", database.customers.ElementAt<Customer>(1), database.customers.ElementAt<Customer>(1).Address, "Hotel hankat skal have en sømme pool insatllert i loftet", new List<User>(), new List<Attachment>()), allTestUser);
            database.addProject(new Project(database.users.First<User>(), "Hankats wellness", database.customers.ElementAt<Customer>(1), database.customers.ElementAt<Customer>(1).Address, "Hotel hankat vil have bygget en wellness område tæt på deres pool", new List<User>(), new List<Attachment>()), allTestUser);
            database.addProject(new Project(database.users.Last<User>(), "Project Tittle", database.customers.Last<Customer>(), database.customers.Last<Customer>().Address, "Project is about this", new List<User>(), new List<Attachment>()), allTestUser);
            database.addProject(new Project(database.users.First(), "Frelsensvandrør", database.customers.First(), "Algade 27 9000 Aalborg", "Fix frelsens vandrør", new List<User>(), new List<Attachment>()), allTestUser);
